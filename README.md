...
| 47 | **Wie heissen die Indikatoren in TradingView?** | Zuordnung Messung → anklickbarer Name. **Eingebaut**: MACD, Stochastic, Bollinger Bands (+ %B, + **Bollinger Bands Width**), **Average True Range**, **Historical Volatility**, Ulcer Index, Relative Volume. **Nicht eingebaut**: Abwaerts-Semivol, EWMA, Parkinson, Garman-Klass → fertig als `tradingview/risiko_panel.pine`. Der staerkste Praediktor (DVOL) ist **kein Indikator, sondern ein Symbol** — Suche: DVOL/BVIV/BitVol, fuer Aktien TVC:VIX | [`tradingview_namen.md`](docs/tradingview_namen.md) |
| 48 | **VWAP getestet** (einziger der vier mit Volumen) | **Als Linie identisch mit der SMA** (+0,9999). Richtung: nichts (t -0,38 bis +0,16). Signal: **12 von 12 Zellen negativ**. Als Filter auf das S/R-Signal: Suche +49→**+89 bp**, Holdout +52→**+26 bp** — **Filter Nr. 10 gescheitert**. **ABER die Bandbreite schlaegt die Bollinger-Breite auf beiden Maerkten** (BTC 0,241 vs 0,200 beim Tageslimit, ETH 0,166 vs 0,148) — und die Zerlegung zeigt: **die Volumengewichtung ist der ganze Effekt**, HLC/3 statt Schluss bringt null | [`vwap.md`](docs/vwap.md) |
...
| 56 | **Die Trader, die es nachweislich koennen** (Hyperliquid, 41.362 on-chain verifizierte Konten) | **„Zweimal hintereinander profitabel" ist REINER ZUFALL**: 26,1 % tatsaechlich gegen 26,3 % bei Unabhaengigkeit. **Aber die Raender persistieren**: oberstes Dezil zweimal = 1,88 % gegen 1,00 % Zufall (**1,9x**), unterstes Dezil 20,0 % gegen 10 %. Kerngruppe (42 Konten) gegen Gegengruppe (29): Frequenz, Ordergroesse, Marktzahl **identisch** — der einzige Unterschied ist der **Taker-Anteil 38,0 % gegen 66,3 %** (p = 0,057). Frequenz trennt nicht: 18,8 % der Kerngruppe handeln unter 20 Fills/Tag. Grundverteilung: 41,9 % im Plus, 4,89 % ueber 1 Mio $, oberstes 1 % haelt **57,7 % aller Gewinne** | [`echte_trader.md`](docs/echte_trader.md) |
| 57 | **Markov-Ketten und der Rainbow Chart** | **Markov: Gedaechtnis ist da** (p = 0,00, Cramers V 0,13, **15 von 16 Faellen out-of-sample bestaetigt**, Log-Loss bis +1,85 %) — **aber netto -15 bis -16 bp bei 16 bp Kosten**, der Bruttovorteil liegt unter 1 bp. **Rainbow Chart: der 3-Monats-Effekt faellt von 48,0 auf 1,1 Prozentpunkte**, sobald man nur Vergangenheitsdaten fittet — 98 % war Look-ahead. Der 1-Jahres-Effekt (+182 Pp) hat **n_eff = 2,4** (t 14,12 → **0,74**, Kontrolle p = 0,138) und beschreibt zwei Zyklen. Die Trendlinie selbst wanderte von Exponent 0,039 auf 0,786; **sieben Jahresprognosen, sieben Fehlschlaege** | [`markov_rainbow.md`](docs/markov_rainbow.md) |
| 58 | **„Den Gamblern Liquiditaet stellen"** (7,8 Mio echte Trades) | Realisierter Spread des Makers direkt gemessen: **-0,575 bp je Trade**, 20 von 20 Tag/Horizont-Zellen negativ. **Die Hypothese kippt**: kleine Trades sind die **schlechtesten** Gegenparteien (-0,715 bp) statt der besten, grosse die besten (-0,246 bp); alle acht Tageszeitfenster negativ. **Was rettet, ist allein die Gebuehr**: Standard 1,5 bp → -2,075 bp, Rebate -1,0 bp → **+0,425 bp**. Das Segment unter 500 $ ist 63,3 % der Trades aber nur 1,4 % des Volumens: **-13,2 Mio $/Jahr bei Standardgebuehr, +1,7 Mio $ mit Rebate** — dieselbe Taetigkeit, 15 Mio $ Unterschied | [`liquiditaet_stellen.md`](docs/liquiditaet_stellen.md) |
...
| 64 | **Renko-Trail: Handelsmanagement als Strategie** | **Kein Baustein erzeugt Erwartungswert** (beste Stufe -6,75 bp in der Suche, **-20,70 bp** im Holdout; Zufallseinstiege mit identischem Management liefern dasselbe, Perzentil hoechstens 91,5 %). **Aber:** ohne ausgefuehrten Stop reisst **jeder vierte Trade** das geplante Risiko, der schlimmste um das **26-fache**. Mit Stop 1,6 % und -1,87 R — fuer **0,16 bp je Trade**. Tage ueber dem 3-%-Limit **7,1 → 0,4** | [`renko_trail.md`](docs/renko_trail.md) |
| 65 | **Renko (OHLC, 1 % Box) + MACD + Open Interest** (Chart-Setup des Nutzers) | 🛑 **Fill am Brick-Level schoent um 11,5-37,5 bp je Trade** — derselbe Aufbau zeigt +7,68 bp und ist ehrlich gerechnet **-29,24 bp**. Der harte Stop bringt hier **+24,4 bp** (traeges Ausstiegssignal: Ø Verlust ohne Stop **-2,19 R**, schlimmster **-40,9 R**). Der OI-Filter trennt in **beiden** Zeitraeumen gleichsinnig (+15,5 / +25,1 bp), ist mit t = 0,16 / 1,20 aber zu schwach | [`renko_macd_oi.md`](docs/renko_macd_oi.md) |
| 66 | **SL 3 Bricks / TP 6 Bricks, 1 % Risiko** (Ausstieg des Nutzers) | **CRV 1:2 ist ein fairer Muenzwurf**: von JEDEM Bar aus wird +6 % vor -3 % in **34,7 %** der Faelle erreicht (Theorie 33,3 %, ueber 100.000 Startpunkte). Break-even mit Kosten **35,0 %**, gemessen **31,7 %** — unter dem Muenzwurf. **CAGR -9,0 %** in einem Markt, der im selben Zeitraum **+147 % p.a.** machte. Fuer +20 % CAGR waeren **42,5 %** Trefferquote noetig | [`renko_macd_klammer.md`](docs/renko_macd_klammer.md) |
| 67 | **Gamma Exposure (GEX): Flip, Call-Wall, Put-Wall** (Indikator des Nutzers) | **Alle vier Behauptungen nicht bestaetigt.** Die Vol haengt am **Betrag** des GEX, nicht am Vorzeichen — und abs. GEX korreliert mit **rho = 0,83** schlicht mit dem Open Interest. Kurs kehrt an den Walls in **3 von 4 Faellen seltener** um als an einem Zufallsstrike gleicher Distanz. Einziges Flackern: Flip-nach-positiv auf BTC ueberlebt die Vol-Mittelwertkontrolle (-19,7 pp, p = 0,011) — aber n = 16 und ETH exakt null | [`gex.md`](docs/gex.md) |
| 68 | **200-$-Push auf 15min, 1 Trade/Tag** (Regel des Nutzers) | **Brutto -2,53 bp, netto -11,55 bp** — der Markt gibt null her, die Reibung nimmt den Rest. Bei 0,387 BTC je Trade: **-26.412 $ im Suchzeitraum, davon 20.329 $ Gebuehren (77 %)**. Noetig waeren **58,1 %** Trefferquote (im Holdout **69,7 %**), gemessen **47,3 %** — und damit **unter** dem Zufallseinstieg (49,6 %). Damit die Gebuehr unter 10 % des Ziels bleibt, muesste das Ziel **~6.300 $** gross sein, nicht 200 $ Nachtrag aus einem zweiten Trade: Gebuehr ist **prozentual** (4,00 bp je Seite), aber **Net PnL ist nicht Settled PnL** (Differenz = eine Opening fee, echte Reibung **11,97 bp**). Bei **10x** sind das **-1,11 % der Margin je Trade x 349 Trades = -97,9 % im Jahr** | [`push200.md`](docs/push200.md) |
| 69 | **Liquiditaet + MACD auf 1h und 4h** (Methode des Nutzers, zerlegt) | **Die 4h-Bestaetigung schadet**: -1,29 bp (Suche) und -1,86 bp (Holdout), konsistent, bei halbierter Signalzahl. **Die Liquiditaet traegt als ZIEL** (+2,02 / +2,96 bp), als Einstiegsort nicht (+0,57 / +0,03). Reicht beides nicht: noetig **60,0 %** Trefferquote, gemessen 42-48 %, **Zufallseinstieg 44,1 %** — also ueber allen MACD-Varianten der Suche. Hier ist schon **brutto negativ** (-7 bis -9,4 bp), anders als beim 200-\$-Push | [`liq_macd.md`](docs/liq_macd.md) |
| 70 | **Derselbe Aufbau auf Heikin Ashi** (Nachtrag) | **HA hilft — in 8 von 8 Vergleichen** (+0,05 bis +2,96 bp), konsistent, aber winzig. Erstmals schlaegt eine Stufe den Zufallseinstieg in **beiden** Zeitraeumen (44,5 % gegen 44,1 %, Holdout **49,2 % gegen 46,4 %**) — der Vorsprung in der Suche ist mit 0,4 pp aber nichts. Die 4h-Bestaetigung schadet auch hier (-0,95 bp). 🛑 **Korrektur zur Abrechnungsfalle**: mit HA-Preisen wird das Ergebnis hier **schlechter** (-4 bis -8 bp), nicht besser — das Vorzeichen der Verzerrung haengt vom Strategietyp ab | [`liq_macd_ha.md`](docs/liq_macd_ha.md) |
